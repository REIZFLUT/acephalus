...
import type { PageProps, Collection, FilterView, PinnedNavigationItem } from '@/types';
...
interface EditProps extends PageProps {
    item: PinnedNavigationItemWithRelations;
    collections: Pick<Collection, '_id' | 'name' | 'slug'>[];
    filterViews: Pick<FilterView, '_id' | 'name' | 'slug'>[];
}

export default function PinnedNavigationEdit({ item, collections, filterViews: initialFilterViews }: EditProps) {
    const [filterViews, setFilterViews] = useState<Pick<FilterView, '_id' | 'name' | 'slug'>[]>(initialFilterViews);
    ...
    useEffect(() => {
        if (data.collection_id && data.collection_id !== item.collection_id) {
    ...
    const handleSubmit = (e: FormEvent) => {
        e.preventDefault();
        put(`/settings/pinned-navigation/${item._id}`);
    };
    ...
    return (
        ...
        >
            <div className="max-w-2xl">
                ...
                <Card>
                    <CardHeader>
                        <CardTitle>Edit Pinned Navigation Item</CardTitle>
                        <CardDescription>
                            Update the navigation shortcut settings.
                        </CardDescription>
                    </CardHeader>
                    <CardContent>
